...
impl ModelLoader {
    ...
    /// Load a model from in-memory bytes (no filesystem — for WASM).
    pub fn load_bytes(&self, data: Vec<u8>, format: CRFFormat) -> Result<CRFModel, String> {
        let mut cursor = Cursor::new(data);
        match format {
            CRFFormat::Native => self.load_native_reader(&mut cursor),
            CRFFormat::CRFsuite => self.load_crfsuite_reader(&mut cursor),
            CRFFormat::Auto => {
                let mut magic = [0u8; 4];
                cursor
                    .read_exact(&mut magic)
                ...
                if &magic == NATIVE_MAGIC {
                    self.load_native_reader(&mut cursor)
                } else if &magic == CRFSUITE_MAGIC {
                    self.load_crfsuite_reader(&mut cursor)
                } else {
                    Err(format!("Unknown file format: {:?}", magic))
                }
            }
    ...
    /// CRFsuite parse from any Read+Seek source (shared by file and byte loaders).
    fn load_crfsuite_reader<R: Read + Seek>(&self, reader: &mut R) -> Result<CRFModel, String> {
        let header = self.read_crfsuite_header(reader)?;
        let mut model = CRFModel::new();
        self.read_crfsuite_strings(reader, header.off_labels, header.num_labels, &mut model, true)?;
        self.read_crfsuite_strings(reader, header.off_attrs, header.num_attrs, &mut model, false)?;
        self.read_crfsuite_features(reader, header.off_features, header.num_features, &mut model)?;
        Ok(model)
    }

    /// Native (bincode) parse from any Read source.
    fn load_native_reader<R: Read>(&self, reader: &mut R) -> Result<CRFModel, String> {
        let mut magic = [0u8; 4];
        reader
            .read_exact(&mut magic)
            .map_err(|e| format!("Failed to read magic: {}", e))?;
        ...
        let model: CRFModel = bincode::deserialize_from(reader)
            .map_err(|e| format!("Failed to deserialize model: {}", e))?;
        Ok(model)
    }

    /// Read CRFsuite file header.
    fn read_crfsuite_header<R: Read + Seek>(&self, reader: &mut R) -> Result<CRFsuiteHeader, String> {
        // Magic (4 bytes)
        let mut magic = [0u8; 4];
        reader
            .read_exact(&mut magic)
    ...
    fn read_crfsuite_strings<R: Read + Seek>(
        &self,
        reader: &mut R,
        offset: u32,
        count: u32,
    ...
    fn read_crfsuite_features<R: Read + Seek>(
        &self,
        reader: &mut R,
        offset: u32,
        count: u32,
